...
def fetch_data(data_dir,train):

    if train:
        data_url = os.path.join(data_dir, "cs-train")
        couples = pd.date_range('2017-11-01', '2019-07-01', freq='MS').strftime("%Y-%m").tolist()
        json_data = ["invoices-" + ym + ".json" for ym in couples]
        data_urls = [os.path.join(data_url, tjd) for tjd in json_data]
    else:
        data_url = os.path.join(data_dir, "cs-production")
        couples = pd.date_range('2019-08-01', '2019-12-01', freq='MS').strftime("%Y-%m").tolist()
        json_data = ["invoices-" + ym + ".json" for ym in couples]
        data_urls = [os.path.join(data_url, tjd) for tjd in json_data]
...
def extract_all_datasets(df):
    
    #extract top 10 countries by price
    
    list_of_country = df.groupby("country").sum().sort_values("price",ascending=False).index[0:10].tolist()
    mask = df['country'].isin(list_of_country)
    df_filt = df[mask] 
    
    list_df = []
    for i in list_of_country:
        temp_df = convert_to_ts(df_filt,country=i)
        temp_df["country"] = i
        list_df.append(temp_df)
    
    country_ts = pd.concat(list_df).reset_index(drop=True)
    ts_glob = convert_to_ts(df_filt)
    
    return df_filt,country_ts,ts_glob

def ts_to_monthly(df):
    
    if "country" in df.columns.tolist():
        return df.groupby(["year_month","country"]).sum().reset_index()
    else:
        return df.groupby("year_month").sum().reset_index()
